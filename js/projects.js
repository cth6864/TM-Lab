...
function initHorizontalYearBars(){
  const counts = Array.from(yearBars).map(bar => {
    const title = bar.getAttribute('title') || '';
    const match = title.match(/(\d{4})년\s*(\d+)(건|편)/);

    if(!match) return 0;

    const year = match[1];
    const count = Number(match[2]);
    const unit = match[3] || '건';

    // 화면에 표시될 텍스트: 2008년도 2건
    bar.dataset.label = `${year}년도 ${count}${unit}`;
